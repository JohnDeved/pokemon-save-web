    ...
    <div className="p-6 bg-zinc-900 min-h-screen">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-300 text-sm mb-4">
          When a Pokemon uses a move that matches its type, it gets a 1.5x power bonus (STAB).
          Hover over the moves below to see the power adjustment in the popover.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 max-w-4xl">
        {demoMoves.map((move, index) => (
          <div key={move.id} className="group">
            <PokemonMoveButton
              move={move}
              pokemonTypes={treeckoTypes}
              isExpanded={false}
              opensUpward={index < 2}
            />
            <div className="mt-2 p-2 bg-zinc-800 rounded text-xs text-gray-300">
              <div>Move Type: <span className="text-blue-400">{move.type}</span></div>
              <div>Pokemon Types: <span className="text-green-400">{treeckoTypes.join(', ')}</span></div>
              <div>
                STAB: {treeckoTypes.includes(move.type) ? 
                  <span className="text-green-400">YES (1.5x)</span> : 
                  <span className="text-red-400">NO</span>
                }
              </div>
              <div>
                Power: {move.power} → {treeckoTypes.includes(move.type) ? 
                  <span className="text-green-400">{Math.floor(move.power! * 1.5)}</span> : 
                  move.power
                }
              </div>
            </div>
      ...
      <div className="mt-8 p-4 bg-zinc-800 rounded">
        ...
        <ul className="text-gray-300 text-sm space-y-1">
          <li>• <strong>Vine Whip</strong> (Grass): 45 → <span className="text-green-400">67</span> (STAB applies)</li>
          <li>• <strong>Tackle</strong> (Normal): 40 → 40 (No STAB)</li>
          <li>• <strong>Absorb</strong> (Grass): 20 → <span className="text-green-400">30</span> (STAB applies)</li>
          <li>• <strong>Quick Attack</strong> (Normal): 40 → 40 (No STAB)</li>
        </ul>
      </div>
    </div>
  )
