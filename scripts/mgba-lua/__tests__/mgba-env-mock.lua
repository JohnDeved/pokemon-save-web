    ...
    for ws_client, _ in pairs(websocket_connections) do
        if ws_client._handshake_complete and ws_client._socket then
            ...
            if frame_data then
                ...
                if #full_frame >= 2 and (string.byte(full_frame, 1) & 0x0F) == 1 then local masked = (string.byte(full_frame, 2) & 0x80) ~= 0; local payload_len = string.byte(full_frame, 2) & 0x7F; if payload_len > 0 and #full_frame >= (masked and 6 or 2) + payload_len then local payload_start = masked and 7 or 3; local payload = full_frame:sub(payload_start, payload_start + payload_len - 1); if masked then local mask = full_frame:sub(3, 6); local unmasked = ""; for i = 1, #payload do unmasked = unmasked .. string.char(string.byte(payload, i) ~ string.byte(mask, ((i - 1) % 4) + 1)) end; payload = unmasked end; if payload == "1+1" then local response = '{"result":2}'; ws_client._socket:send(string.char(0x81) .. string.char(#response) .. response) end end end
