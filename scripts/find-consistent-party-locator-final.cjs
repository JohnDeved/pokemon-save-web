...
/**
 * Script to find a consistent way to locate party data in Quetzal ROM hack
 * Using the existing MgbaWebSocketClient implementation
 * 
...
// Import the existing WebSocket client
...
async function main() {
  ...
  try {
    ...
    if (signatures1.length === 1 && signatures2.length === 1) {
      ...
      console.log('   - Algorithm: Scan 0x2020000-0x2030000 for count byte, validate Pokemon at +4 offset')
