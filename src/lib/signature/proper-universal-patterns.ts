...
export const PROPER_UNIVERSAL_PATTERNS: UniversalPattern[] = [
  {
    ...
    extractAddress: (bytes: Uint8Array, offset: number) => {
      ...
      const immediate = (bytes[offset] ?? 0) | ((bytes[offset + 1] ?? 0) << 8)
      ...
      if (poolOffset + 3 >= bytes.length) return 0
      const targetAddr = (bytes[poolOffset] ?? 0) |
                        ((bytes[poolOffset + 1] ?? 0) << 8) |
                        ((bytes[poolOffset + 2] ?? 0) << 16) |
                        ((bytes[poolOffset + 3] ?? 0) << 24)
...

  {
    ...
    extractAddress: (bytes: Uint8Array, offset: number) => {
      if (offset + 1 >= bytes.length) return 0
      const immediate = bytes[offset + 1] ?? 0
...

  {
    ...
    instructionType: 'ARM',
    extractAddress: (bytes: Uint8Array, offset: number) => {
      if (offset + 5 >= bytes.length) return 0
      const immediate = (bytes[offset + 4] ?? 0) | ((bytes[offset + 5] ?? 0) << 8)
...
function findPatternMatches (buffer: Uint8Array, pattern: UniversalPattern): Array<{ offset: number }> {
  ...
  for (let i = 0; i <= buffer.length - patternBytes.length; i += step) {
    ...
    for (let j = 0; j < patternBytes.length; j++) {
      const maskByte = mask[j] ?? 0
      const bufferByte = buffer[i + j] ?? 0
      const patternByte = patternBytes[j] ?? 0
      if (maskByte !== 0 && (bufferByte & maskByte) !== (patternByte & maskByte)) {
        isMatch = false
        break
      }
    }
