...
import { bytesToGbaString, gbaStringToBytes } from './core/utils.js'

// New: Define columns for party table in a single array for maintainability
const PARTY_COLUMNS = [
  { label: 'Slot', width: 5, value: (_p: PokemonDataInterface, i: number) => (i + 1).toString() },
  { label: 'Dex ID', width: 8, value: (p: PokemonDataInterface) => p.speciesId.toString() },
  { label: 'Nickname', width: 12, value: (p: PokemonDataInterface) => p.nickname },
  { label: 'Lv', width: 4, value: (p: PokemonDataInterface) => p.level.toString() },
  { label: 'Ability', width: 8, value: (p: PokemonDataInterface) => p.abilityNumber.toString() },
  { label: 'Nature', width: 10, value: (p: PokemonDataInterface) => p.nature },
  { label: 'Shiny', width: 6, value: (p: PokemonDataInterface) => p.shinyNumber.toString() },
  {
    label: 'HP',
    width: 30,
    value: (p: PokemonDataInterface) => {
      const hpBars = p.maxHp > 0 ? Math.round(20 * p.currentHp / p.maxHp) : 0
      return `[${'█'.repeat(hpBars)}${'░'.repeat(20 - hpBars)}] ${p.currentHp}/${p.maxHp}`
    },
  },
  { label: 'Atk', width: 5, value: (p: PokemonDataInterface) => p.attack.toString() },
  { label: 'Def', width: 5, value: (p: PokemonDataInterface) => p.defense.toString() },
  { label: 'Spe', width: 5, value: (p: PokemonDataInterface) => p.speed.toString() },
  { label: 'SpA', width: 5, value: (p: PokemonDataInterface) => p.spAttack.toString() },
  { label: 'SpD', width: 5, value: (p: PokemonDataInterface) => p.spDefense.toString() },
  { label: 'OT Name', width: 10, value: (p: PokemonDataInterface) => p.otName },
  { label: 'IDNo', width: 7, value: (p: PokemonDataInterface) => p.otId_str },
]
...
const displayPartyPokemon = (party: readonly PokemonDataInterface[]) => {
...
const displayPartyPokemonRaw = (party: readonly PokemonDataInterface[]) => {
...
const displayPartyPokemonGraph = (party: readonly PokemonDataInterface[]) => {
  if (!party.length) return void console.log('No Pokémon found in party.')
  party.forEach((p, i) => {
