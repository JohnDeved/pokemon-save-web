...
const displayPartyPokemon = (party: readonly PokemonData[]) => {
...
const displayPartyPokemonRaw = (party: readonly PokemonData[]) => {
...
const displayColoredBytes = (raw: Uint8Array, fields: [number, number, string][], bytesPerLine = 32) => {
  ...
  while (pos < raw.length) {
    ...
    for (let i = 0; i < lineBytes.length;) {
      const field = fieldForByte[i], idx = pos + i;
      if (field && idx === field[0]) {
        const [s, e, n] = field, color = colorFor(fields.indexOf(field));
        const fieldLen = Math.min(e - s, lineBytes.length - i), width = fieldLen * 3 - 1;
    ...
    for (let j = 0; j < lineBytes.length; ++j) {
      const field = fieldForByte[j], color = field ? colorFor(fields.indexOf(field)) : '';
...
const displayPartyPokemonGraph = (party: readonly PokemonData[]) => {
  if (!party?.length) return void console.log('No Pokémon found in party.');
  party.forEach((p,i)=>{
