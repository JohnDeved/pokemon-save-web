        ...
        {STAT_NAMES.map((statName, index) => {
          ...
          return (
            <div key={statName} className='grid grid-cols-10 gap-1 sm:gap-2 items-center'>
              ...
              <div
                className={`text-center text-sm ${ivClass} ${
                  iv !== MAX_IV ? 'cursor-pointer hover:text-cyan-300 transition-colors' : ''
                }`}
                onClick={iv !== MAX_IV
                  ? () => {
                    handleIvClick(index)
                  ...
                  : undefined}
                onMouseEnter={iv !== MAX_IV
                  ? () => {
                    setHoveredIvIndex(index)
                  }
                  : undefined}
                onMouseLeave={iv !== MAX_IV
                  ? () => {
              ...
              <div
                className={`col-span-2 text-right text-sm ${
                  isShowingPreview ? 'text-cyan-300' : statClass
                } transition-colors`}
              >
