        ...
        <Skeleton.LoadingProvider loading={isLoading}>
            <div className="p-4 space-y-2 text-xs">
                <div className="grid grid-cols-10 gap-2 text-slate-400">
                    <div className="col-span-1"><Skeleton.Text>STAT</Skeleton.Text></div>
                    <div className="col-span-5 text-end"><Skeleton.Text>IV</Skeleton.Text></div>
                    <div className="text-center"><Skeleton.Text>EV</Skeleton.Text></div>
                    <div className="text-center"><Skeleton.Text>BASE</Skeleton.Text></div>
                    <div className="text-right col-span-2"><Skeleton.Text>TOTAL</Skeleton.Text></div>
                </div>
                            ...
                            <div className="col-span-5 flex items-center gap-2">
                                <div className="flex-1 min-w-0">
                                    <div className="w-full bg-slate-800/70 rounded-full h-2 overflow-hidden">
                                        <div
                                            className={
                                                isLoading // just changing the style here makes it feel really smooth in the loading transition
                                                    ? 'bg-slate-700/50 animate-pulse h-full rounded-full w-full'
                                                    : 'bg-gradient-to-r from-cyan-500 to-blue-500 h-full'
                                            }
                                            style={{ width: `${evPercentage}%` }}
                                        />
                                    </div>
                                </div>
                                <span className="text-white w-8 text-right text-xs flex-shrink-0"><Skeleton.Text>{isLoading ? 255 : ev}</Skeleton.Text></span>
                            </div>
