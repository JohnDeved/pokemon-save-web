...
export const PokemonMoveButton: React.FC<MoveButtonProps> = ({
  ...
  const baseTypes = usePokemonStore(
    s => s.partyList.find(p => p.id === s.activePokemonId)?.details?.types ?? []
  )
  ...
  const effectiveTypes =
    megaPreviewEnabled && Array.isArray(megaTypes) && megaTypes.length > 0 ? megaTypes : baseTypes
  ...
  return (
    <div ref={rootRef} className="relative" onWheel={onWheel}>
      <div
        className={cn(
          'w-full text-left p-3 rounded-lg bg-card/50 group-hover:bg-card/70 border shadow-lg transition-all duration-200 cursor-pointer select-none'
        )}
        onClick={onClick}
      ...
      <AnimatePresence>
        {isExpanded && (
          <motion.div
            ...
            className={cn(
              'absolute left-0 right-0 z-50 p-3 bg-popover border rounded-lg shadow-xl text-xs',
          ...
          >
            <div className="flex justify-between text-muted-foreground mb-2 pb-2 border-b border-border/50 -mx-3 px-3">
              ...
              <div>
                <div>
                  Accuracy:{' '}
                  <div className="text-foreground">{move.accuracy ? `${move.accuracy}%` : '—'}</div>
                </div>
              </div>
            </div>
            <ScrollableContainer
              ref={scrollRef}
              className="max-h-[100px] overflow-y-auto mt-2 custom-scrollbar text-muted-foreground leading-relaxed text-xs"
            >
